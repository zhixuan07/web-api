...
import RecipeModal from '../modals/RecipeModal.vue'
...
async function deleteFavorite(props) {
  try {
    const response = await axios.delete(`http://localhost:3004/api/recipe_favourite`, {
      params: {
        idMeal: props.recipe.idMeal,
        uuid: useUser.getUser.uuid
      }
  ...
  <div>
    <div v-if="props.align == 'vertical'">
      <figure class="relative">
        <img
          v-if="props.recipe.strMealThumb"
      ...
      <div class="card-body">
        <h2 class="card-title">{{ props.recipe.strMeal }}</h2>
        <button @click="openModal" class="btn">View Recipe</button>
      </div>
      ...
      <Teleport to="body">
        ...
        >
          <RecipeModal
            :recipe_id="props.recipe.idMeal"
            :recipe_name="props.recipe.strMeal"
            :recipe_instruction="props.recipe.strInstructions"
            :recipe_ingredient="props.recipe"
            :strMealThumb="props.recipe.strMealThumb"
            :recipe_category="props.recipe.strCategory"
            @close="closeModal"
          />
        </div>
      </Teleport>
    ...
    <div v-if="props.align === 'horizontal'" class="bg-white shadow-md rounded-md">
      <div :class="props.css ? props.css : 'flex flex-row gap-3'">
        ...
        <div class="flex flex-col gap-2 relative w-full">
          <h1 class="text-2xl font-bold pl-2">{{ props.recipe.strMeal }}</h1>
      ...
      <Teleport to="body">
        ...
        >
          <RecipeModal
            ...
            :recipe_name="props.recipe.strMeal"
            :recipe_instruction="props.recipe.strInstructions"
            :recipe_ingredient="props.recipe.strIngredients"
            :strMealThumb="props.recipe.strMealThumb"
            :recipe_category="props.recipe.strCategory"
            @close="closeModal"
          />
